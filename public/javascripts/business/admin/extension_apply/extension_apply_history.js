require(['data-grid','dataform','utils','lib_user'],function($$dataGrid,$$dataForm,$$utils,$$libUser){
    ...
    var $dataGrid = $$dataGrid.init('/admin/extension_apply_history.do',$grid,$('#gridTemplate').text());
    ...
    $form.on('loadDataSuccess',function(e,data){
        ...
        if(data.approve_result){
            $(":input[name='approve_result']").css('color','green');
            data.approve_result = "通过";
        }else{
            $(":input[name='approve_result']").css('color','red');
            data.approve_result = "不通过";
        }
        /* 初始化个人名片显示 */
    ...
    $$dataForm.init($form,{
        viewDataSource : "/admin/extension_apply/{{id}}.do",
    });
